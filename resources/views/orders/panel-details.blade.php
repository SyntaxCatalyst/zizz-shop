    ...
    <div class="max-w-2xl mx-auto bg-white dark:bg-gray-800 p-8 rounded-lg shadow-lg my-10 text-white">
        ...
        <div class="space-y-4 bg-gray-900/50 p-6 rounded-lg">
            <div class="flex justify-between items-center">
                <span class="text-gray-400">Link Panel:</span>
                <a href="{{ $settings->pterodactyl_domain }}" target="_blank" class="font-mono text-blue-400 hover:underline">{{ $settings->pterodactyl_domain }}</a>
            </div>
            <div class="flex justify-between items-center">
                <span class="text-gray-400">Username:</span>
                <span class="font-mono">{{ $serverDetails['username'] }}</span>
            </div>
            <div class="flex justify-between items-center">
                <span class="text-gray-400">Password:</span>
                {{-- Kita gunakan Alpine.js untuk fitur copy-paste --}}
                <div x-data="{ password: '{{ $serverDetails['password'] }}', copied: false }" class="flex items-center space-x-2">
                    <span x-text="copied ? 'Tersalin!' : password" class="font-mono"></span>
                    <button @click="navigator.clipboard.writeText(password); copied = true; setTimeout(() => copied = false, 2000)" title="Salin Password">
